...
ADAFRUIT_IO_IMU_FEEDNAME = credentials["ADAFRUIT_IO_IMU_FEEDNAME"]
ADAFRUIT_IO_ACC_FEEDNAME = credentials["ADAFRUIT_IO_ACC_FEEDNAME"]
ADAFRUIT_IO_MAP_FEEDNAME = credentials["ADAFRUIT_IO_MAP_FEEDNAME"]
ADAFRUIT_IO_BATT_FEEDNAME = credentials["ADAFRUIT_IO_BATT_FEEDNAME"]
ADAFRUIT_IO_BP_FEEDNAME = credentials["ADAFRUIT_IO_BP_FEEDNAME"]
...
mqtt_gps_feedname = '{:s}/feeds/{:s}'.format(ADAFRUIT_USERNAME, ADAFRUIT_IO_GPS_FEEDNAME)
mqtt_imu_feedname = '{:s}/feeds/{:s}'.format(ADAFRUIT_USERNAME, ADAFRUIT_IO_IMU_FEEDNAME)
mqtt_acc_feedname = '{:s}/feeds/{:s}'.format(ADAFRUIT_USERNAME, ADAFRUIT_IO_ACC_FEEDNAME)
mqtt_map_feedname = '{:s}/feeds/{:s}'.format(ADAFRUIT_USERNAME, ADAFRUIT_IO_MAP_FEEDNAME)
mqtt_batt_feedname = '{:s}/feeds/{:s}'.format(ADAFRUIT_USERNAME, ADAFRUIT_IO_BATT_FEEDNAME)
mqtt_bp_feedname = '{:s}/feeds/{:s}'.format(ADAFRUIT_USERNAME, ADAFRUIT_IO_BP_FEEDNAME)
...
def web_print(text_in, feed = mqtt_gps_feedname):
    if killThread == 0:
...
if not c.connect(clean_session=False):
    ...
    c.subscribe(mqtt_imu_feedname)
    c.subscribe(mqtt_acc_feedname)
    c.subscribe(mqtt_gps_feedname)
    c.subscribe(mqtt_map_feedname)
    c.subscribe(mqtt_batt_feedname)
    c.subscribe(mqtt_bp_feedname)
